...
impl FsmState for LAUNCHED {
    ...
    fn on_entry(&self, event: &Self::Event, ctx: Self::Ctx) -> Result<Self::Ctx, Self::Error> {
        if let &RocketLauncherEvent::LAUNCH = event {
            let mut new = ctx.clone();
            new.state = domain::RocketLauncherState::Launched;
            Ok(new)
        } else {
            Ok(ctx)
        }
...
mod tests {
    ...
    fn event_not_found_start() {
        ...
        let result = state::LAUNCHED.fire(RocketLauncherEvent::START(10), ctx);
        match result {
    ...
    fn event_not_found_decrement() {
        ...
        let result = state::LAUNCHED.fire(RocketLauncherEvent::DECREMENT, ctx);
        match result {
            Ok((state, ctx)) => panic!("state={:?} ctx={:?}", state, ctx),
    ...
    fn event_not_found_launch() {
        ...
        let result = state::LAUNCHED.fire(RocketLauncherEvent::LAUNCH, ctx);
        match result {
            Ok((state, ctx)) => panic!("state={:?} ctx={:?}", state, ctx),
    ...
    fn event_not_found_abort() {
        ...
        let result = state::LAUNCHED.fire(RocketLauncherEvent::ABORT, ctx);
        match result {
            Ok((state, ctx)) => panic!("state={:?} ctx={:?}", state, ctx),
