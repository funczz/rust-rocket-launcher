use rust_fsm::{FsmState, FsmTransition};

use crate::{
    domain,
    fsm::{RocketLauncherEvent, RocketLauncherFsmError},
};

use super::{ABORTED, LAUNCHED};

#[allow(nonstandard_style)]
#[derive(Debug, Clone)]
pub struct COUNTING;

impl FsmState for COUNTING {
    type Event = RocketLauncherEvent;
    type Ctx = domain::RocketLauncherModel;
    type Error = RocketLauncherFsmError;

    fn as_box(
        &self,
    ) -> Box<dyn FsmState<Event = Self::Event, Ctx = Self::Ctx, Error = Self::Error>> {
        Box::new(Self)
    }

    fn to_transition(
        &self,
        event: &Self::Event,
    ) -> Result<FsmTransition<Self::Event, Self::Ctx, Self::Error>, Self::Error> {
        match event {
            &RocketLauncherEvent::DECREMENT => Ok(FsmTransition::Internal),
            &RocketLauncherEvent::LAUNCH => Ok(FsmTransition::External(LAUNCHED.as_box())),
            &RocketLauncherEvent::ABORT => Ok(FsmTransition::External(ABORTED.as_box())),
            _ => Err(RocketLauncherFsmError::EventNotFoundError {
                event: event.clone(),
            }),
        }
    }

    fn on_entry(&self, event: &Self::Event, ctx: Self::Ctx) -> Result<Self::Ctx, Self::Error> {
        if let &RocketLauncherEvent::START(v) = event {
            let mut new = ctx.clone();
            new.state = domain::RocketLauncherState::Counting;
            new.count = Some(v);
            Ok(new)
        } else {
            Ok(ctx)
        }
    }

    fn on_do(&self, event: &Self::Event, ctx: Self::Ctx) -> Result<Self::Ctx, Self::Error> {
        if let &RocketLauncherEvent::DECREMENT = event {
            let mut new = ctx.clone();
            new.state = domain::RocketLauncherState::Counting;
            match new.count {
                None => {
                    let state = Self.as_box();
                    let event = event.clone();
                    let message = "count error: None".to_string();
                    let err = RocketLauncherFsmError::Error {
                        state,
                        event,
                        message,
                    };
                    return Err(err);
                }
                Some(v) if v == 0 => {
                    let state = Self.as_box();
                    let event = event.clone();
                    let message = format!("count error: Some({})", v);
                    let err = RocketLauncherFsmError::Error {
                        state,
                        event,
                        message,
                    };
                    return Err(err);
                }
                Some(v) => new.count = Some(v - 1),
            }
            Ok(new)
        } else {
            Ok(ctx)
        }
    }

    fn on_exit(&self, _event: &Self::Event, ctx: Self::Ctx) -> Result<Self::Ctx, Self::Error> {
        Ok(ctx)
    }
}

#[cfg(test)]
mod tests {
    use rust_fsm::FsmState;

    use crate::{
        domain,
        fsm::{state, RocketLauncherEvent},
    };

    #[test]
    fn event_not_found_start() {
        let ctx = domain::RocketLauncherModel {
            state: domain::RocketLauncherState::Ready,
            count: None,
        };
        let result = state::COUNTING.fire(RocketLauncherEvent::START(10), ctx);
        match result {
            Ok((state, ctx)) => panic!("state={:?} ctx={:?}", state, ctx),
            Err(e) => assert_eq!(format!("{}", e), "event not found: START(10)"),
        }
    }

    #[test]
    fn decrement() {
        let ctx = domain::RocketLauncherModel {
            state: domain::RocketLauncherState::Counting,
            count: Some(10),
        };
        let result = state::COUNTING.fire(RocketLauncherEvent::DECREMENT, ctx);
        match result {
            Ok((state, ctx)) => {
                assert_eq!(format!("{:?}", state), "COUNTING");
                assert_eq!(ctx.count.unwrap(), 9);
            }
            Err(e) => panic!("{:?}", e),
        }
    }

    #[test]
    fn decrement_error() {
        let ctx = domain::RocketLauncherModel {
            state: domain::RocketLauncherState::Counting,
            count: Some(0),
        };
        let result = state::COUNTING.fire(RocketLauncherEvent::DECREMENT, ctx);
        match result {
            Ok((state, ctx)) => panic!("state={:?} ctx={:?}", state, ctx),
            Err(e) => assert_eq!(
                e.to_string().as_str(),
                "state: COUNTING, event: DECREMENT, message: count error: Some(0)"
            ),
        }
    }

    #[test]
    fn launch() {
        let ctx = domain::RocketLauncherModel {
            state: domain::RocketLauncherState::Counting,
            count: Some(0),
        };
        let result = state::COUNTING.fire(RocketLauncherEvent::LAUNCH, ctx);
        match result {
            Ok((state, ctx)) => {
                assert_eq!(format!("{:?}", state), "LAUNCHED");
                assert_eq!(ctx.count.unwrap(), 0);
            }
            Err(e) => panic!("{:?}", e),
        }
    }

    #[test]
    fn abort() {
        let ctx = domain::RocketLauncherModel {
            state: domain::RocketLauncherState::Counting,
            count: Some(10),
        };
        let result = state::COUNTING.fire(RocketLauncherEvent::ABORT, ctx);
        match result {
            Ok((state, ctx)) => {
                assert_eq!(format!("{:?}", state), "ABORTED");
                assert_eq!(ctx.count.unwrap(), 10);
            }
            Err(e) => panic!("{:?}", e),
        }
    }

    #[test]
    fn event_not_found_finish() {
        let ctx = domain::RocketLauncherModel {
            state: domain::RocketLauncherState::Ready,
            count: None,
        };
        let result = state::COUNTING.fire(RocketLauncherEvent::FINISH, ctx);
        match result {
            Ok((state, ctx)) => panic!("state={:?} ctx={:?}", state, ctx),
            Err(e) => assert_eq!(format!("{}", e), "event not found: FINISH"),
        }
    }
}
